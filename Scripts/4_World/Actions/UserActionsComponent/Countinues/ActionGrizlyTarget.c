class ActionGrizlyDIVTargetCB : ActionContinuousBaseCB
{
	override void CreateActionComponent()
	{
		m_ActionData.m_ActionComponent = new CAContinuousTime(15.0);
	}
};

class ActionGrizlyDIVTarget : ActionContinuousBase
{
	void ActionGrizlyDIVTarget()
	{
		m_CallbackClass 	= ActionGrizlyDIVTargetCB;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (target && item)
		{
			if (!target.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_GRIZLY))
			{
				target.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_GRIZLY);
			}
