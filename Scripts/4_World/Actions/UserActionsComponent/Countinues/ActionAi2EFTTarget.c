class ActionAi2DIVTargetCB : ActionContinuousBaseCB
{
...
class ActionAi2DIVTarget : ActionContinuousBase
{
	void ActionAi2DIVTarget()
	{
		m_CallbackClass 	= ActionAi2DIVTargetCB;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (target && item)
		{
			if (!target.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_AI2))
			{
				target.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_AI2);
			}
