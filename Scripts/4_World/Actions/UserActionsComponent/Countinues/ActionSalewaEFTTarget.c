class ActionSalewaDIVTargetDIVCB : ActionContinuousBaseCB
{
...
class ActionSalewaDIVTarget : ActionContinuousBase
{
	void ActionSalewaDIVTarget()
	{
		m_CallbackClass 	= ActionSalewaDIVTargetDIVCB;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (target && item)
		{
			if (!target.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_SALEWA))
			{
				target.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_SALEWA);
			}
