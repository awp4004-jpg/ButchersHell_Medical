class ActionIFAKSDIVSelfCB : ActionContinuousBaseCB
{
...
class ActionIFAKSDIVSelf : ActionContinuousBase
{
	void ActionIFAKSDIVSelf()
	{
		m_CallbackClass 	= ActionIFAKSDIVSelfCB;
		m_FullBody 			= false;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (player && item)
		{
			if (!player.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_IFAK))
			{
				player.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_IFAK);
			}
