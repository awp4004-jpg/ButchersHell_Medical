class ActionGrizlyDIVSelfCB : ActionContinuousBaseCB
{
...
class ActionGrizlyDIVSelf : ActionContinuousBase
{
	void ActionGrizlyDIVSelf()
	{
		m_CallbackClass 	= ActionGrizlyDIVSelfCB;
		m_FullBody 			= false;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (player && item)
		{
			if (!player.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_GRIZLY))
			{
				player.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_GRIZLY);
			}
