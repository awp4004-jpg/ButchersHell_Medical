class ActionIFAKSDIVTargetCB : ActionContinuousBaseCB
{
...
class ActionIFAKSDIVTarget : ActionContinuousBase
{
	void ActionIFAKSDIVTarget()
	{
		m_CallbackClass 	= ActionIFAKSDIVTargetCB;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (target && item)
		{
			if (!target.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_IFAK))
			{
				target.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_IFAK);
			}
