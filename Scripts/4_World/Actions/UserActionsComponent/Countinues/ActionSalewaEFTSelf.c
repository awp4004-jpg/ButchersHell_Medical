class ActionSalewaDIVSelfCB : ActionContinuousBaseCB
{
...
class ActionSalewaDIVSelf : ActionContinuousBase
{
	void ActionSalewaDIVSelf()
	{
		m_CallbackClass 	= ActionSalewaDIVSelfCB;
		m_FullBody 			= false;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (player && item)
		{
			if (!player.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_SALEWA))
			{
				player.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_SALEWA);
			}
