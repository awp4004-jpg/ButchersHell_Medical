class ActionAi2DIVSelfCB : ActionContinuousBaseCB
{
...
class ActionAi2DIVSelf : ActionContinuousBase
{
	void ActionAi2DIVSelf()
	{
		m_CallbackClass 	= ActionAi2DIVSelfCB;
		m_FullBody 			= false;
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (player && item)
		{
			if (!player.GetModifiersManager().IsModifierActive(new_DIV_eModifiers.MDF_HEALING_AI2))
			{
				player.GetModifiersManager().ActivateModifier(new_DIV_eModifiers.MDF_HEALING_AI2);
			}
