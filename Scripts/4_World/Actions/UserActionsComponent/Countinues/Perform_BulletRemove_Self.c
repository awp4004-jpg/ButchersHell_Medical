class Perform_BulletRemove_SelfCB : ActionContinuousBaseCB
{
...
class Perform_BulletRemove_Self : ActionContinuousBase
{	
	void Perform_BulletRemove_Self()
	{
		m_CallbackClass 	= Perform_BulletRemove_SelfCB;
	...
	{	
		...
		if (action_data.m_MainItem && target)
		{
			...
			{
				//target.MessageStatus("The surgery failed!");
				return;
			}
