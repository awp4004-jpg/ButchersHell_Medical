class HealingSalewaMdfr : ModifierBase
{
    const int LIFETIME = 120;              // Total duration
    const int HEAL_DURATION = 70;          // Time it actually heals
    const float HEAL_PER_SECOND = 1.0;     // HP per second

    override void Init()
    {
        m_TrackActivatedTime = true;
        m_IsPersistent = false;
        m_ID = new_DIV_eModifiers.MDF_HEALING_SALEWA;
