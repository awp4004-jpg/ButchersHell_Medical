class HealingGrizlyMdfr : ModifierBase
{
    const int LIFETIME = 180;               // Total duration
    const int HEAL_DURATION = 70;          // Time it actually heals
    const float HEAL_PER_SECOND = 1.5;     // HP per second

    override void Init()
    {
        m_TrackActivatedTime = true;
        m_IsPersistent = false;
        m_ID = new_DIV_eModifiers.MDF_HEALING_GRIZLY;
